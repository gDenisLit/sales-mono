...
import { CompanyListItem } from '@/pages/company/components/CompanyListItem'

const PAGE_SIZE = 20
type CompanyListSearch = {
    name: string
    page: number
}
...
export function CompanyListPage() {
    ...
    const { name, page } = useSearch({
        from: '/company/list',
    }) as CompanyListSearch
    ...
    return (
        <section className="space-y-6">
            <header className="flex flex-col gap-2">
                <p className="text-sm uppercase tracking-wide text-blue-300">Results for “{name}”</p>
                <h2 className="text-2xl font-semibold text-white">
                    {isFetching ? 'Loading companies…' : `${total} matching compan${total === 1 ? 'y' : 'ies'}`}
                </h2>
                <p className="text-sm text-slate-400">Select a company to jump into its org chart preview.</p>
            </header>

            <div className="space-y-3">
                {results.map((company) => (
                    <CompanyListItem key={company.id} company={company} onSelect={handleSelectCompany} />
                ))}
